...
type (
	...
	// Notice notice
	Notice struct {
		NotifyTime     string `schema:"notify_time,omitempty" url:"notify_time,omitempty"`           // 是 时间
		NotifyType     string `schema:"notify_type,omitempty" url:"notify_type,omitempty"`           // 是 类型
		NotifyID       string `schema:"notify_id,omitempty" url:"notify_id,omitempty"`               // 是 校验ID
		SignType       string `schema:"sign_type,omitempty" url:"-"`                                 // 是 RSA2
		Sign           string `schema:"sign,omitempty" url:"-"`                                      // 是 签名
		TradeNo        string `schema:"trade_no,omitempty" url:"trade_no,omitempty"`                 // 是 交易号
		APPID          string `schema:"app_id,omitempty" url:"app_id,omitempty"`                     // 是 应用ID
		OutTradeNo     string `schema:"out_trade_no,omitempty" url:"out_trade_no,omitempty"`         // 是 单号
		OutBizNo       string `schema:"out_biz_no,omitempty" url:"out_biz_no,omitempty"`             // 否 业务ID
		BuyerID        string `schema:"buyer_id,omitempty" url:"buyer_id,omitempty"`                 // 否 买家帐号
		BuyerLogonID   string `schema:"buyer_logon_id,omitempty" url:"buyer_logon_id,omitempty"`     // 否 买家帐号
		SellerID       string `schema:"seller_id,omitempty" url:"seller_id,omitempty"`               // 否 卖家帐号
		SellerEmail    string `schema:"seller_email,omitempty" url:"seller_email,omitempty"`         // 否 卖家邮箱
		TradeStatus    string `schema:"trade_status,omitempty" url:"trade_status,omitempty"`         // 否 交易状态
		TotalAmount    string `schema:"total_amount,omitempty" url:"total_amount,omitempty"`         // 否 订单金额
		ReceiptAmount  string `schema:"receipt_amount,omitempty" url:"receipt_amount,omitempty"`     // 否 实收金额
		InvoiceAmount  string `schema:"invoice_amount,omitempty" url:"invoice_amount,omitempty"`     // 否 开票金额
		BuyerPayAmount string `schema:"buyer_pay_amount,omitempty" url:"buyer_pay_amount,omitempty"` // 否 付款金额
		PointAmount    string `schema:"point_amount,omitempty" url:"point_amount,omitempty"`         // 否 积分金额
		RefundFee      string `schema:"refund_fee,omitempty" url:"refund_fee,omitempty"`             // 否 退款金额
		SendBackFee    string `schema:"send_back_fee,omitempty" url:"send_back_fee,omitempty"`       // 否 实际退款
		Subject        string `schema:"subject,omitempty" url:"subject,omitempty"`                   // 是 标题
		Body           string `schema:"body,omitempty" url:"body,omitempty"`                         // 否 描述
		GmtCreate      string `schema:"gmt_create,omitempty" url:"gmt_create,omitempty"`             // 否 创建时间
		GmtPayment     string `schema:"gmt_payment,omitempty" url:"gmt_payment,omitempty"`           // 否 付款时间
		GmtRefund      string `schema:"gmt_refund,omitempty" url:"gmt_refund,omitempty"`             // 否 退款时间
		GmtClose       string `schema:"gmt_close,omitempty" url:"gmt_close,omitempty"`               // 否 结束时间
	}
	// Query query
	Query struct {
		Response
